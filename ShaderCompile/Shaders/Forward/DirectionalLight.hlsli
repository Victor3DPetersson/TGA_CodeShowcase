...
float3 EvaluateDirectionalLight(float3 albedoColor, float3 specularColor, float3 normal, float roughness, float4 lightColor, float3 lightDir, float3 viewDir)
{
	...
	return saturate(lightColor.rgb * lambert * (cDiff * (1.0f - cSpec) + cSpec) * PI * lightColor.a);
