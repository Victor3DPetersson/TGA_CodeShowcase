...
PixelOutput main(VertexToPixel input)
{
    ...
    if(depth == 1)
    {
        output.myColor = float4(globalAmbientLightColor.rgb * globalAmbientLightColor.a, 1);
        return output;
    }
    ...
    //const float sliceScalingFactor = (float) 24.f / log2(renderCamera.farPlane / renderCamera.nearPlane);
    //const float sliceBiasFactor = -((float) 24.f * log2(renderCamera.nearPlane) / log2(renderCamera.farPlane / renderCamera.nearPlane));
    //const uint clusteredPosZ = 24.0 * log(viewDepth / renderCamera.nearPlane) / log(renderCamera.farPlane / renderCamera.nearPlane);
    ...
    output.myColor.rgb = lerp((ambiance + directionalLight) + spotLightInfluence + pointLighInfluence, emissive.rgb * (emissive.a * 15.f), (emissive.a));
    output.myColor.a = 1.0f;
    //output.myColor.rgb = float3(ssao, 0, 0);
    return output;
    }