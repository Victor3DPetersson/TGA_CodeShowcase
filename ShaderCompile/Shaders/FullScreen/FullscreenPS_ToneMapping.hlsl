...
float3 tonemap_s_gamut3_cine(float3 c)
{
    ...
    return mul(s_curve(mul(c, fromSrgb)), toSrgb);
}
...
PixelOutput main(VertexToPixel input)
{
    ...
    float3 pixel = materialTexture1.Sample(defaultSampler, input.myUV).rgb;
    pixel = tonemap_s_gamut3_cine(pixel);

    result.myColor.rgb = pixel; 
